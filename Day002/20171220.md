...
In Target Hosts, `enter your list of host names`, one per line. `hadoop-m2.mos`
...
`Enter the user name for the SSH key you have selected.` If you do not want to use root, you must provide the user name for an account that can execute sudo without entering a password. If SSH on the hosts in your environment is configured for a port other than 22, you can change that also.
